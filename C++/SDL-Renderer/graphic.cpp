...
Graphic::Graphic(unsigned x, unsigned y, std::string const &title):
  _x(x),
  _y(y),
  _screen(NULL)
{
...
void Graphic::PutPixel(int x, int y, Uint32 px)
{
...
void Graphic::SafePutPixel(int x, int y, Uint32 px)
{
  if (_screen && x >= 0 && x < _screen->w && y >= 0 && y < _screen->h)
    this->PutPixel(x, y, px);
}

void	Graphic::drawSquare(int x, int y, unsigned color, unsigned size)
{
  SDL_Rect rect = {x, y, size, size};
  SDL_FillRect(_screen, &rect, color);
}

void Graphic::drawLine(int x1,int y1, int x2,int y2, unsigned color)
{
...
void Graphic::drawCircle(int n_cx, int n_cy, int radius, Uint32 pixel)
{
...
void Graphic::fillCircle(int cx, int cy, int radius, Uint32 pixel)
{
